function alpha_vector = numerical_poincare_ankleos(t,tn,Sn,peakLocs,numMaps)

  [k,n] = size(Sn);         % k-number of samples, n-states
  T_N = numel(peakLocs);    % number of Periods

  % Create time query points
  % Spit each period into numMaps points.
  for i=1:(T_N-1)
    tq(:,i) = linspace(t(peakLocs(i)),t(peakLocs(i+1)),numMaps);
  end
